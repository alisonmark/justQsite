	...
	jQuery(document).ready(function($) {
		if ($('.widget').size() > 0) {
			...
			$('.widget').each(function(index, e) {
				var $this = $(this);
				if ($this.attr('id')) {
					if ($this.attr('id').indexOf('belle_') != -1) $this.find('.widget-title').css({
...
*************************************************************/
	...
	jQuery(document).ready(function($) {
		...
		var revsliderCleanUI = (extData.canonOptionsPost['revslider_clean_ui'] == 'checked') ? true : false;
	
		// bouncer
		if (!revsliderCleanUI) { return; }

		// remove extra revslider notice plugins page
		var $updateMsgs = $('.update-message');

		$updateMsgs.each(function (index, e) {
			var $this = $(this);
			var thisText = $this.text();

			if (thisText.indexOf("Activate Slider Revolution for") != -1) {
				$this.closest('tr').remove();
			}

			if (thisText.indexOf("A new version of Slider Revolution is available") != -1) {
				$this.closest('tr').remove();
			}
